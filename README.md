...
| Some complex scripts are not reordered | Every GSUB lookup type applies, contextual and chaining substitution included, in the stages HarfBuzz uses, and GPOS attaches marks to bases, ligatures and other marks. The syllables of Devanagari, Bengali, Gurmukhi, Gujarati, Oriya, Tamil, Telugu, Kannada and Malayalam are reordered as HarfBuzz reorders them. Sinhala, Khmer, Myanmar, Tibetan and the other scripts that HarfBuzz shapes with its Universal Shaping Engine are not, and Thai sara am is not decomposed. A letter and a combining mark in decomposed form are not composed into one glyph, as HarfBuzz composes them. |
