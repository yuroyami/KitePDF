...
| Indic scripts are not reordered | Every GSUB lookup type applies, contextual and chaining substitution included, in the stages HarfBuzz uses, and GPOS attaches marks to bases, ligatures and other marks. Devanagari and the other Brahmic scripts also need a shaper that reorders each syllable, and Thai needs sara am decomposed, which are not implemented. |
