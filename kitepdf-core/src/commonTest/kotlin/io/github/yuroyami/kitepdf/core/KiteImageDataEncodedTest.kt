...
/**
 * [KiteImageData.fromEncodedImage]: the public entry other handlers (EPUB, CBZ)
 * use to build an image from a self-contained encoded file.
 */
class KiteImageDataEncodedTest {
    ...
    fun jpeg_dimensions_are_sniffed_and_bytes_kept() {
        val img = KiteImageData.fromEncodedImage(jpeg48x32)
        assertTrue(img != null)
    ...
    fun jpeg_dimensions_survive_a_leading_app0_segment() {
        ...
        val img = KiteImageData.fromEncodedImage(app0)
    ...
    fun header_only_png_returns_null() {
        ...
        assertNull(KiteImageData.fromEncodedImage(pngSig))
    }
