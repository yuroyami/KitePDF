...
/**
 * Scores the Skia canvas on every [GradientFixtures] page against mutool, each
 * within its own budget. Skips without mutool.
 */
class SkiaGradientOracleTest {
    ...
    fun every_gradient_fixture_is_within_budget() {
        ...
        for (f in GradientFixtures.all()) {
            val png = PdfPageRasterizer.encodeToPng(KitePDF.open(f.bytes).pages[0], 1.0)
