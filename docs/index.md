...
kotlin {
    sourceSets {
        commonMain.dependencies {
            implementation("io.github.yuroyami:kitepdf:0.3.0")
        }
    }
}
```
...
Every artifact is at `0.3.0`. For a single format, add `kitepdf-pdf` (the PDF handler alone) or `kitepdf-epub` (the EPUB reader alone). The `kitepdf` umbrella contains both. See [Show it on screen](#show-it-on-screen) for each binding in use.

!!! note "Not using Kotlin Multiplatform?"
    The same artifact works in a plain Android or JVM project. Add `io.github.yuroyami:kitepdf:0.3.0` to your normal `dependencies { }` block.
...
```kotlin
implementation("io.github.yuroyami:kitepdf-compose-viewer:0.3.0")
```
...
```kotlin
implementation("io.github.yuroyami:kitepdf-native-renderer:0.3.0")  // or kitepdf-skia-renderer
```
