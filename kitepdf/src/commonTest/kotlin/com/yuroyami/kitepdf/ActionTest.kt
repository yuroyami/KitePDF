...
class ActionTest {
    ...
    fun parses_gotor_action() {
        val dict = dict(
            ...
            "D" to PdfArray(listOf(com.yuroyami.kitepdf.parser.PdfInt(2), PdfName("Fit"))),
            "NewWindow" to com.yuroyami.kitepdf.parser.PdfBoolean(true),
        )
        val a = PdfAction.parse(dict, noResolver)
    ...
    fun parses_uri_action() {
        val dict = dict(
            ...
            "IsMap" to com.yuroyami.kitepdf.parser.PdfBoolean(true),
        )
        val a = PdfAction.parse(dict, noResolver)
    ...
    fun parses_submit_and_reset_form_actions() {
        val submit = PdfAction.parse(
            dict(
                ...
                "Flags" to com.yuroyami.kitepdf.parser.PdfInt(4),
            ),
            noResolver,
        )
        ...
        val reset = PdfAction.parse(
            dict("S" to PdfName("ResetForm"), "Flags" to com.yuroyami.kitepdf.parser.PdfInt(0)),
            noResolver,
        )
    ...
    fun unknown_action_type_falls_through_to_unknown() {
        val dict = dict("S" to PdfName("Sound"), "Volume" to com.yuroyami.kitepdf.parser.PdfReal(0.7))
        val a = PdfAction.parse(dict, noResolver)
