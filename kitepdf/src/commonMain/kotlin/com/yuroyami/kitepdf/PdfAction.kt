            ...
            return when (s) {
                ...
                "JavaScript" -> {
                    val script = when (val js = dict["JS"]) {
                        ...
                        is PdfStream -> com.yuroyami.kitepdf.filters.FilterChain.decode(js).decodeToString()
                        is PdfReference -> when (val resolved = refs.resolve(js)) {
                            is PdfString -> resolved.asText()
                            is PdfStream -> com.yuroyami.kitepdf.filters.FilterChain.decode(resolved).decodeToString()
                            else -> ""
                        }
                        else -> ""
                    }
