 ...
 * (ISO 32000-1 §7.3) — the inverse of [com.yuroyami.kitepdf.parser.Parser].
 *
 * The output is designed to round-trip exactly through KitePDF's own
 * [Lexer]/[com.yuroyami.kitepdf.parser.Parser] and to be accepted by other
...
object PdfObjectWriter {
    ...
    fun writeObject(obj: PdfObject, out: ByteArrayBuilder) {
        when (obj) {
            ...
            is PdfInt -> out.ascii(obj.value.toString())
            ...
            is PdfReference -> out.ascii("${obj.objectNumber} ${obj.generation} R")
        }
    }
    ...
    private fun writeStream(stream: PdfStream, out: ByteArrayBuilder) {
        ...
        val dictMap = LinkedHashMap(stream.dict.map)
        dictMap["Length"] = PdfInt(stream.rawBytes.size.toLong())
        writeDictionary(PdfDictionary(dictMap), out)
        out.ascii("\nstream\n")
