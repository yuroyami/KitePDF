 ...
 *
 * Fonts are referenced through [fontResolver], which maps a [StandardFont] to
 * the resource name the owning [PdfBuilder] will register in `/Resources`.
 */
class ContentStreamBuilder internal constructor(
    ...
    private fun num(d: Double) { out.append(PdfObjectWriter.formatReal(d).encodeToByteArray()); sp() }
    ...
    private fun op(operator: String): ContentStreamBuilder { out.append(operator.encodeToByteArray()); return nl() }
