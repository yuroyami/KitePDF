...
object Aes {
    ...
    private fun mixColumns(state: IntArray) {
        for (c in 0..3) {
            ...
            state[c * 4]     = (gmul(s0, 2) xor gmul(s1, 3) xor s2 xor s3) and 0xFF
            state[c * 4 + 1] = (s0 xor gmul(s1, 2) xor gmul(s2, 3) xor s3) and 0xFF
            state[c * 4 + 2] = (s0 xor s1 xor gmul(s2, 2) xor gmul(s3, 3)) and 0xFF
            state[c * 4 + 3] = (gmul(s0, 3) xor s1 xor s2 xor gmul(s3, 2)) and 0xFF
        }
    }
    ...
    private fun invMixColumns(state: IntArray) {
        for (c in 0..3) {
            val s0 = state[c * 4]; val s1 = state[c * 4 + 1]
            val s2 = state[c * 4 + 2]; val s3 = state[c * 4 + 3]
            state[c * 4]     = (gmul(s0, 0x0E) xor gmul(s1, 0x0B) xor gmul(s2, 0x0D) xor gmul(s3, 0x09)) and 0xFF
            state[c * 4 + 1] = (gmul(s0, 0x09) xor gmul(s1, 0x0E) xor gmul(s2, 0x0B) xor gmul(s3, 0x0D)) and 0xFF
            state[c * 4 + 2] = (gmul(s0, 0x0D) xor gmul(s1, 0x09) xor gmul(s2, 0x0E) xor gmul(s3, 0x0B)) and 0xFF
            state[c * 4 + 3] = (gmul(s0, 0x0B) xor gmul(s1, 0x0D) xor gmul(s2, 0x09) xor gmul(s3, 0x0E)) and 0xFF
        }
    }
