...
class PdfFont private constructor(
     ...
     */
    fun layoutBytes(bytes: ByteArray): List<TextGlyph> {
        composite?.let { return layoutComposite(it, bytes) }
        return layoutSimple(bytes)
    }

    private fun layoutSimple(bytes: ByteArray): List<TextGlyph> {
        ...
        for ((offset, byteVal) in bytes.withIndex()) {
            ...
            val outline = simpleOutline(code)
    ...
    private fun layoutComposite(c: CompositeFont, bytes: ByteArray): List<TextGlyph> {
        val out = ArrayList<TextGlyph>(bytes.size / 2 + 1)
        for (unit in c.codeUnits(bytes)) {
            val outline = c.outline(unit.cid)
    ...
    private fun simpleOutline(code: Int): PdfPath? {
        ...
        embeddedTtf?.let { ttf -> return ttf.outline(simpleGid(code))?.toPdfPath() }
